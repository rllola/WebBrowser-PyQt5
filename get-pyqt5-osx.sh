...
$PYTHON_EXE configure.py --confirm-license --enable=QtWidgets --enable=QtWebEngineWidgets --enable=QtCore --qmake=$QMAKE_EXE -n PyQt5.sip --sip="$($PYTHON_EXE-config --prefix)/bin/sip"
make -j $NP
sudo make install
echo "Done !"
